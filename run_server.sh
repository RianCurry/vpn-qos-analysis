...
SAMPLER_SCRIPT=${SAMPLER_SCRIPT_PATH:-"/home/raspian/vpn-qos-analysis/collect_pi_metrics.sh"}
...
run_mode() {
    ...
    # Cek iperf3 server
    echo "[CHECK] Memeriksa iperf3 server di $TARGET:5201"
    timeout 5 iperf3 -c "$TARGET" -t 1 &>/dev/null
    if [ $? -ne 0 ]; then
        echo "[WARN] iperf3 server tidak merespons. Pastikan server berjalan."
    fi
    ...
    for LEVEL in $LEVELS; do
        ...
        for i in $(seq 1 "$ITER"); do
            ...
            # Tunggu file CSV muncul
            if wait_for_file "$SYS_FILE"; then
                echo "    [SAMPLER] File CSV terdeteksi."
            else
                echo "    [WARN] File CSV tidak muncul, lanjutkan tetap (mungkin sampler bermasalah)"
            fi
            ...
            # 4. Copy CSV (dengan mencoba beberapa kali)
            echo "    [COPY] Menyalin $SYS_FILE ke $RESULT_DIR/"
            COPY_SUCCESS=0
            for attempt in 1 2 3; do
                if pi_scp "$PI_USER@$PI_IP:$SYS_FILE" "$RESULT_DIR/" 2>/dev/null; then
                    echo "    [COPY] Sukses (attempt $attempt)"
                    COPY_SUCCESS=1
                    break
                fi
                echo "    [COPY] Gagal attempt $attempt, tunggu 1 detik..."
                sleep 1
            done
            if [ $COPY_SUCCESS -eq 0 ]; then
                echo "    [WARN] Gagal copy CSV setelah 3 percobaan"
            fi
